...
#根据图像梯度对像素点排序，梯度大的点先被攻击
def grad(images_batch,max_iters):
    batch_size = images_batch.size(0)
    image_size = images_batch.size(2)
    pix_one_channel = image_size * image_size  #图像三维总点数
    #x = torch.zeros(batch_size,pix_one_channel * 3)
    grand_index = [ ]
    for i in range (batch_size): #该模式下，batch_size暂时只能为1
        img = images_batch[i]
        img = img.numpy()
        img = img *255
        ...
        img_grad = cv.Canny(img,80,150)
        img_grad = np.transpose(img_grad).tolist()  #梯度矩阵转置        
        img_grad = np.array(img_grad)
        ########
        '''test = cv.imread('test.jpg')
        gr = cv.Canny(test,80,150)
        ret = np.hstack((img_grad, gr))
        pyplot.imshow(ret)
        pyplot.show()'''
        ########
        pix_gradint_num = { }
        ...
        for row in range(image_size):
            for conl in range(image_size):
                if 255==img_grad[row,conl]:
                    pix_gradint_num[num] = img_grad[row,conl]
                    num = (row * image_size) + conl
        
        '''index = 0
        for key in pix_gradint_num_sort:
            pix_num = int(key[0])
            x[i,index] = pix_num
            x[i,index + 1] = pix_num + pix_one_channel
            x[i,index + 2] = pix_num + (2 * pix_one_channel)
            index = index + 3'''
 

        for key in pix_gradint_num:
            pix_num = int(key)
            grand_index.append(pix_num)
            grand_index.append(pix_num + pix_one_channel)
            grand_index.append(pix_num + (2 * pix_one_channel))
        rd.shuffle(grand_index)  #随机打乱
        #grand_index_num = len(grand_index)
        data_size = 3 * image_size * image_size
        
        if len(grand_index) < (max_iters): 
            insert = random_int_list(1,data_size,data_size) #生成随机数

            for t in range(data_size):   # 将被攻击像素向量维数扩展到max_iters（最大攻击次数）
                rand_insert = insert[t]
                if len(grand_index) < (max_iters): 
                    if(rand_insert not in grand_index):
                        grand_index.append(rand_insert)
                else: 
                    break
        #path.append(mid)
    grand_index = np.array(grand_index)

    grand_index = torch.Tensor(grand_index)
    grand_index= grand_index.long()
    #index_result = grand_index[0,]
    return grand_index
