...
class SalEval(object):
    def __init__(self, nthresh=49):
        ...
        self.thresh = torch.from_numpy(np.linspace(1./(nthresh + 1), 1. - 1./(nthresh + 1), nthresh)).float().cpu()
        self.EPSILON = 1e-8

        self.recall = torch.zeros(nthresh).cpu()
        self.precision = torch.zeros(nthresh).cpu()
        self.mae = 0
        self.num = 0
